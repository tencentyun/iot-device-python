...
运行 [MqttSample.py](../sample/MqttSample.py)，设备成功上线后，成功订阅过Topic后，调用checkFirmware()，订阅检查固件更新的 Topic`$ota/update/${productID}/${deviceName}` ，发布检查固件更新的 Topic `$ota/report/${productID}/${deviceName}` 。示例代码如下：
