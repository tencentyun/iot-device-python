...
-----END CERTIFICATE-----"
    ...
    def shadowJsonConstructReport(self, *args):
        format_string = '"%s":"%s"'
        format_int = '"%s":%d'
        report_json = "{\"type\": \"update\", \"state\": {\"reported\": {"
        arg_cnt = 0

        for arg in args:
            arg_cnt += 1
            if arg.type == "int" or arg.type == "float":
                report_json += format_int % (arg.key, arg.data)
            elif arg.type == "string":
                report_json += format_string % (arg.key, arg.data)
            else:
                self.__explorer_log.error("type not support")
                arg.data = ""
            if arg_cnt < len(args):
                report_json += ","
        pass
        report_json += "}}, \"clientToken\": \"%s\"}"

        client_token = self.__device_file.product_id + "-" + str(self._shadow_token_num)
        self._shadow_token_num += 1

        json_out = report_json % (client_token)
