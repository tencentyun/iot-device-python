...
def example_http():
    logger.debug("\033[1;36m http device report test start...\033[0m")
    ...
    while True:
        if qcloud.isHttpConnected():
            ...
            if count >= 3:
                logger.error("\033[1;31m http device report test fail...\033[0m")
                return False
            time.sleep(1)
            count += 1
    ...
    # qcloud.disconnect()
    logger.debug("\033[1;36m http device report test success...\033[0m")
